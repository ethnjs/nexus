...
# ---------------------------------------------------------------------------
# GET /users/{user_id}/tournament-memberships/ — powers /profile/[id]'s
# tournament sections for a viewer other than the profile owner.
# ---------------------------------------------------------------------------

class TestListUserTournamentMemberships:
    def test_own_memberships_always_visible(self, client, td_user, td_tournament):
        """td_user holds Tournament Director via ownership — no manage_members
        grant needed to see their own row."""
        login(client, "td@test.com", "tdpass")
        res = client.get(f"/users/{td_user.id}/tournament-memberships/")
        assert res.status_code == 200
        assert [m["tournament_id"] for m in res.json()] == [td_tournament.id]

    def test_other_user_visible_with_manage_members(self, client, td_user, td_tournament, other_user, db):
        membership = grant_role(db, td_tournament, other_user, "Volunteer Coordinator")
        login(client, "td@test.com", "tdpass")
        res = client.get(f"/users/{other_user.id}/tournament-memberships/")
        assert res.status_code == 200
        assert [m["id"] for m in res.json()] == [membership.id]

    def test_other_user_hidden_without_manage_members(self, client, td_tournament, other_user, db):
        # td_user is the tournament owner and always sees everything — use a
        # non-owner viewer with a role that lacks manage_members instead.
        viewer = _db_user(db, "runner@example.com")
        grant_role(db, td_tournament, viewer, "Runner")
        grant_role(db, td_tournament, other_user, "Volunteer")
        login(client, "runner@example.com", "Password@1")
        res = client.get(f"/users/{other_user.id}/tournament-memberships/")
        assert res.status_code == 200
        assert res.json() == []

    def test_empty_when_no_memberships(self, client, td_user, other_user):
        login(client, "td@test.com", "tdpass")
        assert client.get(f"/users/{other_user.id}/tournament-memberships/").json() == []
