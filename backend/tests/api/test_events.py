...
def test_create_trial_event(client: TestClient):
    t = _make_tournament(client)
...
def test_create_event_division_b(client: TestClient):
    t = _make_tournament(client)
...
def test_create_event_invalid_division(client: TestClient):
    t = _make_tournament(client)
    response = _make_event(client, t["id"], division="D")
    assert response.status_code == 422


def test_create_event_invalid_type(client: TestClient):
    t = _make_tournament(client)
    response = _make_event(client, t["id"], event_type="fake")
    assert response.status_code == 422


def test_create_event_invalid_volunteers_needed(client: TestClient):
    t = _make_tournament(client)
    response = _make_event(client, t["id"], volunteers_needed=0)
    assert response.status_code == 422


def test_create_event_duplicate_blocks(client: TestClient):
    t = _make_tournament(client)
    response = _make_event(client, t["id"], blocks=[1, 1, 2])
    assert response.status_code == 422


def test_create_event_invalid_tournament(client: TestClient):
    response = _make_event(client, 9999)
    assert response.status_code == 404


def test_create_duplicate_event_same_division(client: TestClient):
...
def test_create_same_event_different_division(client: TestClient):
...
def test_list_events_invalid_tournament(client: TestClient):
...
def test_update_event_location(client: TestClient):
    t = _make_tournament(client)
    created = _make_event(client, t["id"]).json()
...
def test_update_event_blocks(client: TestClient):
    t = _make_tournament(client)
    created = _make_event(client, t["id"]).json()
    response = client.patch(f"/api/v1/events/{created['id']}", json={
        "blocks": [1, 2, 3]  # only morning blocks
    })
    assert response.status_code == 200
...
def test_delete_tournament_cascades_to_events(client: TestClient):
